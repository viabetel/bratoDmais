...
interface ProductCardProps {
  ...
  /** Opcional: sobrescreve o href do card para incluir contexto de intent (ex: ?intent=rent) */
  intentHref?: string
}
...
export function ProductCard({ product, variant = 'grid', intentHref }: ProductCardProps) {
  ...
  if (variant === 'list') {
    return (
      <Link href={intentHref ?? `/p/${product.slug}`}>
      ...
      <div className="group bg-white border border-gray-200 rounded-xl overflow-hidden hover:shadow-xl hover:border-blue-300 transition-all duration-300 h-full flex flex-col relative">
        ...
        <div className="p-3 flex-1 flex flex-col">
          ...
          <div className="mt-auto space-y-1.5 border-t border-gray-100 pt-2">
            ...
            {/* CTAs secundários de serviço (apenas quando aplicável) */}
            {(serviceTypes.includes('installation') || serviceTypes.includes('rental') || serviceTypes.includes('maintenance')) && (
              <div className="flex gap-1.5 mt-1.5">
                {serviceTypes.includes('installation') && (
                  <Link
                    href={`/p/${product.slug}?intent=install`}
                    onClick={(e) => e.stopPropagation()}
                    className="flex-1 text-center text-[10px] font-semibold text-orange-600 border border-orange-200 bg-orange-50 hover:bg-orange-100 rounded-md py-1 transition-colors"
                  >
                    🔧 Instalar
                  </Link>
                )}
                {serviceTypes.includes('rental') && (
                  <Link
                    href={`/p/${product.slug}?intent=rent`}
                    onClick={(e) => e.stopPropagation()}
                    className="flex-1 text-center text-[10px] font-semibold text-green-700 border border-green-200 bg-green-50 hover:bg-green-100 rounded-md py-1 transition-colors"
                  >
                    📅 Alugar
                  </Link>
                )}
                {serviceTypes.includes('maintenance') && (
                  <Link
                    href={`/p/${product.slug}?intent=maintenance`}
                    onClick={(e) => e.stopPropagation()}
                    className="flex-1 text-center text-[10px] font-semibold text-blue-700 border border-blue-200 bg-blue-50 hover:bg-blue-100 rounded-md py-1 transition-colors"
                  >
                    🛠️ Manutenção
                  </Link>
                )}
